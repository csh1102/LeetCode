...
//
// 
// arrivali is the arrival time of the iᵗʰ customer. The arrival times are 
//sorted in non-decreasing order. 
// timei is the time needed to prepare the order of the iᵗʰ customer. 
// 
//
// When a customer arrives, he gives the chef his order, and the chef starts 
//preparing it once he is idle. The customer waits till the chef finishes preparing 
//his order. The chef does not prepare food for more than one customer at a time. 
//The chef prepares food for customers in the order they were given in the input. 
//
// Return the average waiting time of all customers. Solutions within 10⁻⁵ from 
...
//preparing it immediately at time 1, and finishes at time 3, so the waiting time 
//of the first customer is 3 - 1 = 2.
...
//preparing it immediately at time 5, and finishes at time 7, so the waiting time 
//of the first customer is 7 - 5 = 2.
...
//starts preparing it immediately at time 20, and finishes at time 21, so the waiting 
//time of the fourth customer is 21 - 20 = 1.
...
// 
//
// Related Topics Array Simulation 👍 490 👎 51


//leetcode submit region begin(Prohibit modification and deletion)
double averageWaitingTime(int** customers, int customersSize, int* customersColSize) {
    int *firstCus = customers[0];
    int firstCusArriveTime = firstCus[0];
    int firstCusTimeNeeded = firstCus[1];
    int timer = firstCusArriveTime + firstCusTimeNeeded;
    int customerIndex = 1;
    double sum = firstCusTimeNeeded;
    while(customerIndex < customersSize){
        int *customer = customers[customerIndex];
        int startTime = customer[0];
        int needed = customer[1];
        if(timer <=  startTime){
            timer = startTime + needed;
            sum += needed;
        } else{
            sum += timer - startTime + needed;
            timer += needed;
        }
        customerIndex++;
    }
